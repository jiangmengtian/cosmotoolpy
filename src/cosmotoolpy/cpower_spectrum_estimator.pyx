...
def _cross_power_spectrum_estimator_complex128(cnp.ndarray[cnp.complex128_t, ndim=3] delta_k1, cnp.ndarray[cnp.complex128_t, ndim=3] delta_k2, int Ngrid, double L) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Compute the cross power spectrum of two density contrast fields in Fourier space for type(delta_k) == np.complex128
    '''
    cdef double kF = 2*pi/L
    cdef double V  = L*L*L

    cdef int kx_size = delta_k1.shape[0]
    cdef int ky_size = delta_k1.shape[1]
    cdef int kz_size = delta_k1.shape[2]
    cdef int nx_max = kx_size // 2
    cdef int ny_max = ky_size // 2
    cdef int nz_max = kz_size
    cdef int n_max = <int>(round(sqrt(<double>(nx_max*nx_max+ny_max*ny_max+nz_max*nz_max)))+1)
    ...
    cdef int kx_mid = (kx_size-1)//2
    cdef int ky_mid = (ky_size-1)//2
    cdef cnp.complex128_t[:, :, :] delta_k1_view = delta_k1
    cdef cnp.complex128_t[:, :, :] delta_k2_view = delta_k2
    for i in range(kx_size):
        nx = i-kx_size if i>kx_mid else i
        for j in range(ky_size):
            ny = j-ky_size if j>ky_mid else j
...
def cross_power_spectrum_estimator(delta_k1: np.ndarray, delta_k2: np.ndarray, Ngrid: int, L: float = 1000) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Compute the cross power spectrum of two density contrast fields in Fourier space
    ...
    k: 1d-array
        Wavenumber of each bin
    P_k: 1d-array
        Averaged cross power spectrum of each bin
    N_k: 1d-array
        Number of independent modes in each bin
    '''
    if delta_k1.dtype == np.complex128 and delta_k2.dtype == np.complex128:
        return _cross_power_spectrum_estimator_complex128(delta_k1, delta_k2, Ngrid, L)
    else:
        raise TypeError(f"Unsupported input type: {delta_k1.dtype}, {delta_k2.dtype}")